...
export function occurrenceToDescription(
  ...
  // Wrap photos in <button> so mobile browsers (esp. iOS) reliably fire activation events;
  // bare <img> taps often do nothing inside the Cesium InfoBox iframe.
  const photoBox =
    validUrls.length > 0
